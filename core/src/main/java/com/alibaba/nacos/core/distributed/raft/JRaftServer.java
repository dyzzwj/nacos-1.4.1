...
public class JRaftServer {
    ...
    //处理ReadRequest。这里调用sofa-jraft的Node.readIndex方法处理一致性读，如果处理失败，会降级走一遍raft流程（过半提交，应用log，和写流程一样）
    ...
    //如果当前是leader节点，那么不用经过rpc调用，可以直接执行写入操作。
    // （follower重定向过来也是由同样的方法处理）可以看到，这里使用sofa-jraft的Node.apply(Task)方法提交本次写入请求。
